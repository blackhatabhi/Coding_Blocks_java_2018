...
//		Scanner Scn = new Scanner(System.in);
//		int n = Scn.nextInt();
//		System.out.println(gt.justlarger(n));
...
// question of lecture_ 20; 
	...
	public static int nodecount(Node root, int x) {
		int count = 0;
		if (root == null) {
			return 0;
		}
		...
		for (Node child : root.children) {
			count = count + nodecount(child, x);
		}
		return count;

	}

	public int justlarger(int item) {
		return justlarger(this.root, item);
	}

	private static int justlarger(Node node, int n) {
		...
		if (node.data > n) {
			nextlargernode = node.data;
		}
		for (Node child : node.children) {
			int nextlargerinchild = justlarger(child, n);
			if (nextlargerinchild != 0) {
				if (nextlargernode == 0 || nextlargernode > nextlargerinchild) {
...
//		if (!structurallyIdentical(tnode.children.get(i), onode.children.get(i))) {
//			return false;
//		}
//	}
//	return true;
